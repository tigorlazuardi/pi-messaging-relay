...
- real-server idle text delivery through two extension instances, exact no-option `sendUserMessage`, unique IDs, exact ACK correlation, redacted logs, and deterministic `received` output.
