...
The connected transport derives the canonical body budget from exact fixed send-frame overhead, rejects incrementally before output exceeds the general 512 KiB ceiling, and retains a final exact complete-frame check. This is not the distinct UTF-8 262,144-byte serialized-body boundary, stable operation-level rejection, and connection-reuse acceptance still owned by ticket #24.
</Decision>
