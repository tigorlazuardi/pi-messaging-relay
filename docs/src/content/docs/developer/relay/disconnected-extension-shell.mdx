...
The model cannot provide `message_id`, delivery policy, steering intent, or any other relay field. Addresses remain opaque strings. Schema validation rejects extra properties, an empty destination, array or null bodies, and missing required fields before a connected implementation can own those values. The schema retains the accepted final-v1 object shape, while the staged #14 runtime rejects an object locally with `object_body_unsupported` before generating IDs or writing to the socket. Ticket #16 enables that shape with deterministic rendering.

<Decision title="Defer the 256 KiB body outcome to ticket #24" status="accepted">
The connected transport rejects any complete outgoing text frame above the general 512 KiB ceiling and never forwards an unbounded frame. Ticket #24 still owns the distinct UTF-8 262,144-byte serialized-body boundary, its stable operation-level rejection, and connection-reuse acceptance. Object transport and canonicalization remain #16.
</Decision>
...
- real-server text delivery through two extension instances, exact no-option idle injection and busy `{ deliverAs: "followUp" }` injection, stale-session and idle-predicate failure safety, thrown-Pi-call best-effort ACK, sender schema rejection of delivery-mode fields, no steering, unique IDs, exact ACK correlation, redacted logs, and deterministic `received` output.
