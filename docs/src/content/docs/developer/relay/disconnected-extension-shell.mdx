...
flowchart TD
    ...
    A --> R[interruption: bounded jittered reconnect with same session route]
    R --> A
    A --> X[session_shutdown: cancel timer, join attempt, settle retained socket]
```

The synchronous factory registers the command, both tools, and the two lifecycle handlers. Factory execution does not start timers, open sockets, call `fetch`, inspect a model, or inject session content. `session_start` makes one immediate bounded authentication attempt when both endpoint and installation key exist. A successful `/relay-pair CODE` invocation performs bounded file and HTTP work and, when the session is already active, makes one pairing-owned immediate authentication attempt. Failed attempts and retained-socket loss schedule at most ten single-flight retries with the fixed policy in [automatic session reconnect](/developer/relay/automatic-reconnect/). `session_shutdown` idempotently cancels its timer, closes and joins an in-flight attempt, closes a retained socket, and rejects an in-flight list or send operation.

<Decision title="Keep factory loading inert and give session lifecycle socket ownership" status="accepted">
Loading only registers surfaces and lifecycle handlers, so it creates no resource that outlives a factory invocation. `session_start` and successful pairing during an active session start connection ownership; one active session lifecycle owns all later reconnect deadlines and attempts; `session_shutdown` cancels and joins that ownership.
</Decision>
...
Authentication establishes a live session identity. One retained-socket owner correlates either `list_peers` or `agent_send`, accepts validated recipient pushes, and serializes writes. It never queues a second outgoing operation. Its closed send-result parser accepts `denied/sender_capacity` for compatible-server completeness even though normal single-flight use cannot fill the server allowance.
</Decision>
...
The focused Node test loads the real extension factory behind a strict fake Pi host. The fake records command and tool registrations, notifications, hidden custom session entries, `sendMessage` attempts, and `sendUserMessage` attempts. It exposes only controlled session-entry reads plus an idle/busy predicate; unknown Pi or context access fails the test, so model registry and other live capabilities cannot be consulted silently.
...
- no Pi message injection or unapproved live context access while disconnected;
...
- fake-clock reconnect progression with deterministic jitter, exact same-session route/address retention, authenticated presence disappearance and restoration, fresh new/fork routes, and no post-shutdown timer or attempt; and
...
The server owns authoritative send settlement with transport correlation IDs, route addresses, latency, and `body: "<redacted>"`. The extension separately emits `relay_send_settled` for the completed `agent_send` tool operation with the retained authenticated sender route, message ID, requested recipient route, status, reason, latency, and the same body marker; it neither claims stronger Pi receipt nor exposes server-owned delivery or transport request IDs. Failed model-tool attempts—including local `body_too_large`—write one `relay_operation_failed` warning with `result: "failed"`, a stable bounded reason, and only safe available fields; a body field, when applicable, is `<redacted>`. Pairing and session/reconnect lifecycle use the vocabulary in [Structured relay diagnostics](./structured-diagnostics/). Pairing-code, nonce, signature, private-key, and body content never enter logs. The accepted v1 observability decision excludes traces and metrics for this small operator-run relay, so this shell adds neither spans nor counters.
