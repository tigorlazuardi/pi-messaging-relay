...
description: How online string and object delivery renders deterministic provenance, preserves busy recipient work, and acknowledges the Pi attempt.
status: accepted
date: 2026-09-11
---
...
When a valid `re` value is supplied, `, re=<original-id>` appears after the message ID. Rendering `re` does not implement the correlated reply workflow; ticket #17 still owns creation and independent settlement of a new reply message.
...
Busy follow-up routing is implemented by #15. Ticket #16 adds this provenance and object-rendering boundary. A safe local object is encoded canonically before the send frame is created; the server forwards the accepted object body as the same JSON value without interpreting or rebuilding it; and the recipient canonicalizes it for Pi text injection. Optional valid `re` is rendered, but reply behavior remains #17. No string body, object content, or rendered provenance header enters an error or log.
