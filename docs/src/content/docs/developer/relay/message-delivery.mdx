...
description: How online sends, immediate offline results, and correlated replies route and settle at the Pi boundary.
status: accepted
date: 2026-09-11
---
...
This seam connects `agent_send` to one exact currently published authenticated destination, immediately settles an exact current-registry miss as offline, or settles an offered delivery whose exact ACK misses its five-second deadline. It implements successful string and JSON-object delivery for both idle and busy recipients. Addresses remain whole opaque registry keys; cwd, hostname, route formatting, model input, and message identifiers never grant routing authority.
...
At the sender extension boundary, the closed allowlist contains only exact `{message_id,status:"received"}`, `{message_id,status:"timeout",reason:"offline"}`, and `{message_id,status:"timeout",reason:"ack_timeout"}` payloads with matching correlation. Denied results, future timeout reasons, missing or extra fields, and any other pair fail content-free and terminally close the socket. If #20 or #24 later implements another sender outcome, that work must explicitly expand both the result type and parser allowlist; no additional outcome is accepted today.

A route that was published and then removed receives exactly the same result as a syntactically valid opaque route never published by this process. The server stores no route history, tombstone, existence bit, offline inbox, or replay state. The only ticket #18 boundary is the miss at that exact lookup. If a recipient is found there and disappears before or during the write, ticket #20 still owns settlement; a successfully written offer that remains unacknowledged now follows the bounded ACK-timeout path.
...
Ticket #20 owns recipient-disconnected settlement after a successful registry lookup, and #21 owns immediate sender-disconnect cleanup assertions. The #20 found-then-lost path may still end without `send_result`; the extension's eight-second send response deadline then closes its socket. Retry and idempotence (#22), dedupe (#23), the distinct 256 KiB body rule (#24), 128-send pipelining (#25), reconnect (#26), restart authentication (#27), replay behavior (#28), and additional diagnostics (#29) are also deferred.
</Aside>
