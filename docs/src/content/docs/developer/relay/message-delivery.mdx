...
<Aside type="caution" title="Diagnostics remain deferred">
Bounded automatic reconnect restores authenticated live presence, including authorization from the allowlist after a normal server restart. The recipient delivery-stream acceptance now proves that reconnect and process replacement emit no pending, settled, or historical message frames. Additional diagnostics (#29) remain deferred; this no-replay seam adds no event vocabulary.
</Aside>
