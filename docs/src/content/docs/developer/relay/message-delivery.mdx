...
A raw authenticated client can submit an over-limit body inside an otherwise valid frame. Strict decode preserves its validated request and message correlation while retaining no body copy. Before dedupe observation or fingerprinting, sender-capacity reservation, dedupe insertion, recipient lookup, delivery-ID generation, pending state, timer, or write, the relay returns exact `{message_id,status:"denied",reason:"body_too_large"}` through bounded response ownership. The sender and recipient stay healthy. Because lookup never occurs, the denial event omits `recipient_route`.
...
<Decision title="Keep repeat handling before distinct-send capacity" status="accepted">
The sole reader classifies already-owned message identity before capacity. Pending identical retries attach and retained replays/conflicts reserve immediate responses without another slot or worker. Only dedupe-new legal sends attempt one of 128 exact-session reservations. One 256-slot response sequencer covers original and attached retry responses for every admitted send and serializes their writes and audits without an unbounded mailbox.
</Decision>
...
At the sender extension boundary, the closed allowlist contains only exact `{message_id,status:"received"}`, `{message_id,status:"timeout",reason:"offline"}`, `{message_id,status:"timeout",reason:"recipient_disconnected"}`, `{message_id,status:"timeout",reason:"ack_timeout"}`, `{message_id,status:"denied",reason:"message_id_conflict"}`, `{message_id,status:"denied",reason:"body_too_large"}`, and `{message_id,status:"denied",reason:"sender_capacity"}` payloads with matching correlation. The extension accepts server body/capacity denials for compatibility, although local body validation and one-outgoing-operation single-flight mean its ordinary traffic cannot reach either server boundary. Other denied results, future timeout reasons, missing or extra fields, and any other pair fail content-free and terminally close the socket. The model result never exposes the server-only delivery ID.
...
Each authenticated serve owns one sole reader, one bounded response sequencer, at most 128 send workers, and one terminal close function. Every serve exit marks the exact session unavailable once, cancels its per-serve context, force-closes the socket, and joins every send worker plus the response sequencer before returning. A terminal parser failure has a priority transition: it atomically stops normal response publication, revokes routing through the per-entry write fence, and cancels producers. Claimed normal work still waiting on its producer start gate is discarded; an already-active normal write/audit may finish. If revocation rejects that normal write, the sequencer treats it as terminal progress rather than exiting. If an already-written claimed response instead fails its audit after terminal admission has won, fatal reporting remains owned by that audit failure but the sequencer still preserves terminal progress. It discards queued normal work with slot release, then gives exactly one bounded protocol-error write/audit attempt to the same sole writer before hard close. A normal write or audit failure that wins before terminal admission keeps the existing immediate fatal close. This applies to protocol, dispatcher, response-encoding, response-write, audit, client-close, and shutdown exits.

Strict protocol and body-size classification occur before capacity. Legal sends then observe bounded dedupe state; repeats attach or answer without another worker. A dedupe-new send attempts one nonblocking exact-session reservation. The first 128 start concurrent dispatch workers. A further distinct send is denied as `sender_capacity` without lookup or retained operation state. Capacity is released once: immediately when no response can be delivered, or only after the original and optional attached-retry responses own bounded sequencer positions.

The response sequencer has exactly 256 ownership slots, enough for two responses from every admitted logical send. Every original/attached batch reserves its exact one or two slots before encoding either response, and every immediate outcome reserves one before encoding. Preparation, cancellation, or terminal-publication failure releases the exact reservation once. Thus no more than 256 encoded unanswered normal responses exist; one terminal protocol error is separately bounded. The sequencer serializes writes and audit events. The shared start gate for one logical send stays closed until all its response reservations are encoded and published and its capacity slot is released, so peer-visible settlement cannot race ahead of the bounded release point. Immediate outcomes execute in the reader and wait for sequencer ownership when full, applying WebSocket backpressure instead of spawning goroutines or growing a queue.

A `list` also executes synchronously, one at a time, in the sole reader. It can overlap pending sends, consumes no SEND capacity, and has no server-side list queue. A validated `received` frame has no response and likewise dispatches synchronously; it can settle any matching held send for the session's recipient role. Exact recipient-session and delivery/message-ID checks still decide whether an ACK settles anything.
...
Lock order is intentionally one-way at the boundary. Send installation holds the dispatcher mutex and briefly takes the registry mutex for the exact identity check. Registry revocation first marks the entry invisible under the registry mutex, releases it, acquires the per-entry write lease, and only then briefly reacquires the registry mutex to clear identity. After releasing both, it serializes the lifecycle callback and takes the dispatcher mutex. Final writes acquire the entry lease before their brief registry revalidation and hold no registry mutex during network I/O. Shutdown likewise releases the registry mutex before calling the dispatcher. No path waits on the dispatcher or performs I/O while retaining the registry mutex, so write, cleanup, removal, and shutdown cannot form a registry/dispatcher lock cycle.

<Decision title="Abandon exact sender-owned pending work without a public result" status="accepted">
A disconnected sender cannot receive a trustworthy correlated result. Immediate exact-session deletion releases memory, all owned SEND reservations, and ACK waiters without adding a reason, replaying work, logging a misleading settlement, or disturbing recipient presence and other senders. Response-free ACK candidates and bounded lists stay serial and non-retained in the reader while distinct sends use at most 128 joined workers.
</Decision>
...
<Aside type="note" title="Diagnostics preserve no-replay ownership">
Bounded automatic reconnect restores authenticated live presence, including authorization from the allowlist after a normal server restart. Reconnect events describe scheduling and authentication only; they emit no pending, settled, or historical message frame. See [Structured relay diagnostics](./structured-diagnostics/) for the captured-stream contract and redaction evidence.
</Aside>

## Lifecycle no-replay acceptance

One continuous real-public-boundary scenario pairs a sender and recipient extension behind fake Pi hosts, then follows the same recipient delivery stream through exact socket loss and replacement of the real Go relay child on the same address and durable allowlist state. It records every offered delivery ID, generated message ID, body marker, recipient socket generation, and Pi injection in order.

The scenario first settles one message, then withholds the next recipient ACK and terminates that exact recipient socket. The sender receives `timeout/recipient_disconnected`; after automatic reauthentication, bounded roster and quiet-turn checkpoints prove neither message appears again before one fresh post-reconnect send delivers normally. It then withholds another ACK while the server shuts down gracefully. The sender operation is abandoned by connection loss, with no fabricated settlement. After the replacement child starts and both extensions reauthenticate, the same checkpoints prove that no settled, interrupted, startup, or history delivery appears before one fresh post-restart send delivers normally. Held ACK frames are intercepted rather than delayed, so they cannot leak into replacement sockets.

The test also confirms every body remains absent from relay and extension logs, every existing settlement event keeps `body: "<redacted>"`, and the durable allowlist contains no body, message ID, delivery ID, pending, dedupe, inbox, or route state. Disk inspection is subordinate evidence only; recipient frames, tool outcomes, authentication, roster synchronization, and Pi calls are the public acceptance boundary.
...
<Tabs>
  ...
  <TabItem label="Raw sender capacity">
    ```text
    input: exact sender session owns 128 in-flight distinct sends, then submits another dedupe-new legal send
    output: {"message_id":"01993c84-fc2b-7e1c-af99-61b8118ac6df","status":"denied","reason":"sender_capacity"}; socket remains open
    ```
  </TabItem>
