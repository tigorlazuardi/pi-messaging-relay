...
This seam connects `agent_send` to one exact currently published authenticated destination or immediately settles an exact current-registry miss as offline. It implements successful string and JSON-object delivery for both idle and busy recipients. Addresses remain whole opaque registry keys; cwd, hostname, route formatting, model input, and message identifiers never grant routing authority.
...
At the sender extension boundary, the current closed allowlist contains only exact `{message_id,status:"received"}` and `{message_id,status:"timeout",reason:"offline"}` payloads with matching correlation. Denied results, other timeout reasons, missing or extra fields, and any other pair fail content-free and terminally close the socket. If #19, #20, or #24 later implements another sender outcome, that work must explicitly expand both the result type and parser allowlist; no additional outcome is accepted today.

A route that was published and then removed receives exactly the same result as a syntactically valid opaque route never published by this process. The server stores no route history, tombstone, existence bit, offline inbox, or replay state. The only ticket #18 boundary is the miss at that exact lookup. If a recipient is found there and disappears before or during the write, ticket #20 still owns settlement; ACK expiry remains ticket #19.
...
Ticket #17 verifies the successful independent `received` path. Ticket #18 now gives an offline reply destination the same immediate `timeout/offline` result as any ordinary send, without checking whether the original message or route ever existed. ACK expiry and recipient disconnect after lookup remain deferred to tickets #19 and #20.
...
Ticket #19 owns the ACK-expiry result, #20 owns recipient-disconnected settlement after a successful registry lookup, and #21 owns immediate sender-disconnect cleanup assertions. Until #19 and #20 land, those found-recipient paths may end without `send_result`; the extension's exact five-second response deadline then closes its socket. The relay still removes pending work at its exact five-second ACK ceiling, on recipient removal, or on shutdown, but ACK expiry itself returns no sender result. Retry and idempotence (#22), dedupe (#23), the distinct 256 KiB body rule (#24), 128-send pipelining (#25), reconnect (#26), restart authentication (#27), replay behavior (#28), and additional diagnostics (#29) are also deferred.
</Aside>
