---
title: Authenticated text delivery
description: How online text delivery selects idle or busy recipient-local Pi injection, acknowledges the attempt, and preserves active work.
status: accepted
date: 2026-09-11
---
...
This seam connects `agent_send` to one exact currently published authenticated destination. It implements successful text delivery for both idle and busy recipients. Addresses remain whole opaque registry keys; cwd, hostname, route formatting, model input, and message identifiers never grant routing authority.
...
sequenceDiagram
    ...
    alt recipient is idle
        ...
        RE->>P: sendUserMessage(text, followUp)
    end
...
Busy follow-up routing is implemented by #15. Provenance headers and stable object rendering are #16. Reply behavior is #17. The final v1 schema still accepts object bodies and optional `re`, but the current runtime delivery is text-only. A local object-body call fails with reason `object_body_unsupported` before IDs or a frame are created, leaving the retained socket available for later text or list operations. A raw-client object send remains syntactically valid at the protocol parser, then ends without an offer, pending state, or interim sender result. A recipient that receives an object push closes without calling Pi or acknowledging it. No object content enters an error or log. Ticket #16 removes these staged guards when it adds the provenance header and canonical compact object renderer.
