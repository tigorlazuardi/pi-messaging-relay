...
The runnable server entry point is `cmd/pi-messaging-relay-server`. It owns the loopback process lifecycle and implements installation pairing, session authentication, connected live `list`/`roster` discovery, successful authenticated idle and busy string/object delivery, immediate offline settlement on a current registry miss, recipient-disconnected settlement after lookup and before ACK, five-second missing-ACK settlement after a recipient offer is written, and immediate sender-owned pending cleanup when that exact sender disconnects.
...
The shared five-second bound first marks the registry closing so in-memory delivery waits cancel immediately, then owns HTTP shutdown and serve settlement before force-closing and settling every reserved or authenticated registry-owned WebSocket. Registry shutdown that claims pending delivery work emits no `recipient_disconnected` result; only recipient cancellation that already won keeps that exact outcome. Exact sender removal releases sender-owned pending offers with no unreachable result or `send_settled`; sender identity wins for self-send. Recipient removal releases every matching pending offer once, and no pending body is persisted or replayed. Registry removal releases its mutex before the lifecycle callback takes the dispatcher mutex, while send installation briefly checks registry identity under the dispatcher mutex, preventing a registry/dispatcher lock cycle during removal or shutdown. It prevents graceful shutdown from waiting forever on either HTTP or registry-owned process resources. Every HTTP response has a two-second write deadline. Structured event delivery uses one worker per stdout or stderr sink, no queued buffer, and a one-second handoff/acknowledgement deadline. Logger close also waits at most one second. A stalled output therefore cannot retain request handlers or process exit indefinitely.

Each authenticated handler also owns its reader lifetime independently of process shutdown. One terminal function marks the session unavailable once, closes admission in the mutex-protected state machine, cancels the per-serve context, force-closes the WebSocket, and joins both its sole reader and sole response-sequencer workers before the handler returns. Thus dispatcher, response-write, audit, protocol, client-close, and shutdown failures cannot leave a reader or worker goroutine behind. The sequencer has exactly four total ownership slots, performs every authenticated response write and matching audit serially, and fails closed on unsupported overflow. Four covers the exact conforming overlap: the prior logical operation's original/retry pair may remain active and queued after its first response becomes peer-visible, while the next logical operation reserves its own pair. Each operation's original and attached retry reserve ownership before admission publication; their shared start gate prevents either response becoming visible early.

A second distinct complete list/send atomically claims either already-published post-dispatch admission or terminal close; it never waits for future admission. An exact repeat of the current sender-session/message identity is classified first and can attach its one request correlation without claiming a second dispatch. Response-free ACK candidates execute serially in the reader to preserve a session's overlapping recipient role. Ledger and registry locks protect only state decisions: no socket write, log write, blocking channel send, or other I/O occurs while either is held. Registry lifecycle releases its registry lock before dedupe and delivery cleanup.

<Decision title="Join every authenticated reader and settlement owner before handler return" status="accepted">
Socket close plus context cancellation makes a blocked WebSocket reader or response write interruptible; completion joins keep ownership explicit. Admission publication at dispatcher completion permits one bounded ordered response pair to overlap the next sole dispatcher and its possible response pair. Four fixed ownership slots cover that exact sequence. No second distinct pending send, per-frame goroutine, or unbounded application queue exists. Ticket #25 remains deferred.
</Decision>
