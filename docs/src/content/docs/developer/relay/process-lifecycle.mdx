...
Each authenticated handler also owns its reader lifetime independently of process shutdown. One terminal function marks the session unavailable once, closes admission in the mutex-protected state machine, cancels the per-serve context, force-closes the WebSocket, and joins both reader and any bounded response-settlement worker before the handler returns. Thus dispatcher, response-write, audit, protocol, client-close, and shutdown failures cannot leave a reader or worker goroutine behind. A second complete list/send atomically claims either already-published post-dispatch admission or terminal close; it never waits for future admission. Response-free ACK candidates execute serially in the reader to preserve a session's overlapping recipient role.

<Decision title="Join every authenticated reader and settlement owner before handler return" status="accepted">
Socket close plus context cancellation makes a blocked WebSocket reader or response write interruptible; completion joins keep ownership explicit. Admission publication at dispatcher completion permits one bounded ordered response settlement to overlap the next sole dispatcher without creating a second pending send or an application queue. Ticket #25 remains deferred.
</Decision>
