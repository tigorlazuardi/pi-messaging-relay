...
The runnable server entry point is `cmd/pi-messaging-relay-server`. It owns the loopback process lifecycle and implements installation pairing, session authentication, and connected live `list`/`roster` discovery for authenticated sessions. Message routing and delivery/acknowledgment outcomes remain deferred to ticket #14; offline outcomes remain deferred to ticket #18.
