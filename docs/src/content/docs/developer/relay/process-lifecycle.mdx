...
The runnable server entry point is `cmd/pi-messaging-relay-server`. It owns the loopback process lifecycle and now hosts the installation-pairing endpoint. WebSocket messaging, session authentication, and routing remain unimplemented.
...
flowchart TD
    ...
    R --> C[Remove temporary state; retain explicit durable state]
    C --> E[Emit server_stopped with graceful result]
    E --> X[Exit zero]
```

The five-second bound prevents graceful shutdown from waiting forever on a process resource. Every HTTP response has a two-second write deadline. Structured event delivery uses one worker per stdout or stderr sink, no queued buffer, and a one-second handoff/acknowledgement deadline. Logger close also waits at most one second. A stalled output therefore cannot retain request handlers or process exit indefinitely.

A pairing audit failure is stored in a queryable first-error latch and enters the bounded HTTP drain. Main checks that latch again after drain and before any successful stop event, so concurrent SIGINT, SIGTERM, or injected cancellation cannot mask it. Main then attempts `server_failed` through the separately bounded stderr worker and exits non-zero; explicit durable state is retained, and an accepted handler still returns its already-persisted identity.
...
<Decision title="Bound event sinks and latch pairing audit failure" status="accepted">
One writer worker per sink preserves event order without an unbounded queue. One-second write acknowledgement and close deadlines bound handlers and shutdown. The first fatal audit error remains queryable after notification, so termination races cannot produce `server_stopped` or exit zero.
</Decision>
