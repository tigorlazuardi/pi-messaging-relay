...
The runnable server entry point is `cmd/pi-messaging-relay-server`. It owns the loopback process lifecycle and implements installation pairing, session authentication, connected live `list`/`roster` discovery, and successful authenticated idle text delivery. Offline, missing-ACK, and recipient-disconnect sender results remain deferred to tickets #18–#20.
