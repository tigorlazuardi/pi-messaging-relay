...
`restart.acceptance.test.ts` owns the separate restart seam. It runs the same Linux binary twice against one explicit state directory and one operating-system-selected loopback address. The first child pairs and authenticates a real extension, then exits normally; the second child loads the unchanged allowlist, accepts the extension's fake-clock reconnect with the same client and route identity, exposes it through a second authorized extension's roster, and rejects an independently keyed extension. Exact disk inspection proves only version, client ID, public key, and pairing time survive.
