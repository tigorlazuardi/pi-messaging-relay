...
The runnable server entry point is `cmd/pi-messaging-relay-server`. It owns the loopback process lifecycle and implements installation pairing, session authentication, connected live `list`/`roster` discovery, successful authenticated idle and busy string/object delivery, and immediate offline settlement on a current registry miss. Missing-ACK and recipient-disconnect sender results remain deferred to tickets #19 and #20.
