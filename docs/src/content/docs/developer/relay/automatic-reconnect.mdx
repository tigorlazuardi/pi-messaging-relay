...
An interrupted `list_peers` or `agent_send` still settles through the retained socket's existing disconnected path. Reconnect never resumes that operation and never reuses its request or message state. A caller starts a fresh tool operation only after authentication restores the connection. Across a normal server process restart, the same scheduler reconnects to the replacement listener and authenticates against the reloaded durable allowlist without another pairing command. Ticket #28 still owns broader reconnect/restart no-replay acceptance; this restart seam asserts only fresh authentication and roster state.
