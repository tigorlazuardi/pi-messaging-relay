...
This slice implements `/v1/connect` from challenge through welcome. It retains one authenticated socket for the Pi session but does not add roster responses, delivery, reconnect, or restart acceptance.

<Aside type="note" title="Ticket boundaries">
Ticket #12 owns the complete malformed, duplicate, unknown-field, frame-type, and protocol-major matrix. Ticket #13 owns roster behavior. Ticket #26 owns reconnect and backoff. Ticket #27 owns restart acceptance. This seam performs only the strict parsing and bounded reads needed to authenticate safely.
</Aside>
...
The extension pins `ws` v8.21.3 under the MIT license as a direct runtime dependency, with no required transitive dependencies. The tagged primary API documents client-side `maxPayload` enforcement and `terminate()`, which destroys the transport. Authentication configures `maxPayload: 16384` before connecting and disables compression. Normal cleanup requests a graceful close, waits at most one second, then terminates and awaits the terminal close event. This same owner covers connecting, authenticating, retained, and shutdown states. The package and lock sit beside the extension so Nix-installed Pi resolves `ws` from the extension package instead of assuming Pi internals or a global Node installation.

<Decision title="Use bounded maintained WebSocket libraries on both trust boundaries" status="accepted">
The Go package provides context deadlines and streaming read limits. The extension's pinned client adds a pre-materialization 16 KiB payload ceiling and hard transport termination that the Node global WHATWG client does not expose.
</Decision>
