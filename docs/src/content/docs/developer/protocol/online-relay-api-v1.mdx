...
- `message_id` identifies a logical message and supports reply correlation. It is not a request ID.
...
The server enforces one active `list` or `send` dispatcher at a time per authenticated connection. One mutex-protected state machine owns admission, unsupported-concurrency terminal closure, shutdown, and failure. The initial admission allows the first operation. After its dispatcher returns and any response has a bounded settlement owner, the coordinator atomically publishes the next admission before allowing that response worker to write. The sole reader then atomically either claims an already-published admission for immediate dispatcher handoff or claims terminal closure; it never waits with a decoded operation for future admission.

This dispatcher-completion boundary is deliberate. A WebSocket peer can observe a response before the server's `Write` returns, so response-write completion cannot distinguish a conforming post-response frame from an earlier pipelined frame. If frame B claims before A publishes admission, B is discarded and the session is hard-closed without a new error or operation result. If A publishes first, B is accepted for immediate next dispatch even when A's response settlement is still finishing. Ordered response settlement remains bounded, and there is never more than one active delivery dispatch or pending send for that connection. This is not the 128-send pipelining owned by ticket #25.

`received` is the response-free exception because one authenticated session can simultaneously be a sender and a recipient. The sole reader dispatches each validated ACK candidate synchronously, one at a time, without retaining it or consuming `list`/`send` admission. The delivery dispatcher still accepts only the exact recipient session plus matching delivery and message IDs; unknown, stale, or wrong-session ACKs remain harmless. This exception preserves valid recipient overlap but does not authorize an operation queue.

<Decision title="Linearize operation admission at completed dispatcher ownership" status="accepted">
One synchronization domain gives frame claim and admission publication a deterministic winner. Dispatcher completion—not peer-visible response timing—publishes the next admission because the latter is not observable reliably at the server. An admitted frame receives the sole dispatcher immediately; an earlier frame closes instead of waiting. At most one ordered response settlement may overlap the next active dispatcher, so no second pending send or #25 capacity is introduced.
</Decision>
...
A reply uses the same ordinary `agent_send({ to, body, re })` and wire `send` operations. The receiving agent copies `to` from the original header's complete JSON-quoted `from` value and copies `re` from its `id`. The extension generates a fresh request ID and reply `message_id`; the relay generates a fresh delivery ID. The reply ACK and `send_result` settle only that reply call, independently of the original result. The model cannot supply any of those generated IDs. There is no automatic reply, special reply operation, server message-history lookup, or guarantee of completion or another reply. `re` carries correlation only and grants no routing authority. Internal retry remains deferred to ticket #22.
...
- five seconds for the extension's `list_peers` response and eight seconds for its `agent_send` response;
- 1,024 tracked server connections; and
- one use and exactly ten minutes for a pairing code, with the deadline itself expired.

Retry and idempotence (#22), dedupe behavior (#23), the distinct 256 KiB body rule (#24), 128-send pipelining (#25), automatic reconnect (#26), restart authentication (#27), broader restart/reconnect replay assertions (#28), and additional diagnostics (#29) remain deferred. Ticket #21 adds no retry, automatic reconnect, persistence, replay, or public result mechanism. This version publishes no I/O shape or runtime guarantee for those remaining seams.
