...
Operation-level denial is a different state transition: a structurally valid operation reaches the operation dispatcher, a canonical result may be returned, and the parser returns to the next message boundary without closing. Tickets #36 and #13 implement live `list`/`roster` discovery, which remains connected after authentication. Tickets #14 and #15 implement successful idle and busy text paths through exact recipient acknowledgment. Ticket #18 still owns offline outcomes, while tickets #19 and #20 own missing-ACK and recipient-disconnect sender results; those failure outcomes remain disconnected.
...
<Aside type="note" title="Implementation boundary">
Tickets #14 and #15 implement successful string delivery: extension-generated request and message IDs, exact opaque-address routing to a published authenticated session, server-generated delivery ID, current recipient-local `ctx.isIdle()` selection, exact one-argument idle injection, exact busy `{ deliverAs: "followUp" }` injection, exact recipient ACK authorization, and correlated `received`. No sender field can select delivery mode or steer current work. Provenance and object rendering (#16), replies (#17), offline/ACK-timeout/recipient-disconnect results (#18–#20), sender-disconnect assertions (#21), retry/dedupe (#22–#23), 256 KiB body denial (#24), and 128-send pipelining (#25) remain incomplete.
