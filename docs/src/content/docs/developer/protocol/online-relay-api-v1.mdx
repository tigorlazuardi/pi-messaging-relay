...
description: Accepted client/server contract for authenticated pairing, online presence, and successful unicast delivery with correlated replies.
status: accepted
date: 2026-09-08
---
...
Operation-level denial is a different state transition: a structurally valid operation reaches the operation dispatcher, a canonical result may be returned, and the parser returns to the next message boundary without closing. Tickets #36 and #13 implement live `list`/`roster` discovery, which remains connected after authentication. Tickets #14–#16 implement successful idle and busy string/object paths through deterministic provenance and exact recipient acknowledgment. Ticket #18 still owns offline outcomes, while tickets #19 and #20 own missing-ACK and recipient-disconnect sender results; those failure outcomes remain disconnected.
...
<Aside type="note" title="Implementation boundary">
Tickets #14–#17 implement successful string and object delivery plus correlated replies through the same ordinary send path: extension-generated request and message IDs, exact opaque-address routing to a published authenticated session, server-generated delivery ID, raw JSON-value object forwarding, deterministic provenance rendering, current recipient-local `ctx.isIdle()` selection, exact one-argument idle injection, exact busy `{ deliverAs: "followUp" }` injection, exact recipient ACK authorization, and independently correlated `received` results. The reply destination and `re` value are copied from the original injected provenance; no sender field can select a generated ID, delivery mode, or steering.

A reply is never automatic and does not use a special operation. The relay does not look up message history or validate that `re` names a retained message; `re` is correlation metadata, not authorization. Each reply gets a fresh message ID, delivery ID, request/response correlation, ACK, and result independent of the original call. Offline/ACK-timeout/recipient-disconnect results (#18–#20), sender-disconnect assertions (#21), retry/dedupe (#22–#23), 256 KiB body denial (#24), and 128-send pipelining (#25) remain incomplete.

The local extension accepts safe string or JSON-object bodies under its closed tool schema. Unsafe non-JSON runtime graphs and malformed `re` values fail content-free before IDs or socket work. The relay protocol parser remains the strict duplicate/type/depth boundary and forwards an accepted object as the same JSON value. The recipient parser accepts only a string or non-array object and passes sender, message ID, optional valid `re`, and body to the pure renderer. Renderer failure produces neither Pi injection nor ACK and force-settles the retained socket.
</Aside>
...
The only finalized sender outcome for delivery is the successful path documented above: the intended recipient extension sends the exact `received` acknowledgment and the relay returns the correlated `send_result` with `status: "received"`.

Offline delivery (#18), ACK expiry (#19), and recipient disconnect (#20) do not yet produce a final machine-readable `send_result`. The relay releases retained work after its exact five-second ACK ceiling and may release it earlier when the recipient disconnects or the server shuts down; expiry itself returns no sender result. The sender extension waits its own exact five-second response deadline, then rejects the local call with `Relay agent_send response timed out.` and closes the retained socket.
