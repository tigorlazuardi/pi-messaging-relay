...
description: Accepted client/server contract for authenticated pairing, online presence, unicast delivery, correlated replies, and immediate offline results.
status: accepted
date: 2026-09-08
---
...
<Aside type="note" title="Implementation boundary">
Tickets #14–#18 implement successful string and object delivery, correlated replies, and immediate current-registry-miss settlement through the same ordinary send path: extension-generated request and message IDs, exact opaque-address lookup, no history-derived routing, server-generated delivery IDs only after a recipient is found, raw JSON-value object forwarding, deterministic provenance rendering, current recipient-local `ctx.isIdle()` selection, exact one-argument idle injection, exact busy `{ deliverAs: "followUp" }` injection, exact recipient ACK authorization, independently correlated `received` results, and exact `timeout/offline` miss results. The reply destination and `re` value are copied from the original injected provenance; no sender field can select a generated ID, delivery mode, or steering.

A reply is never automatic and does not use a special operation. The relay does not look up message history or validate that `re` names a retained message; `re` is correlation metadata, not authorization. Each online reply gets a fresh message ID, delivery ID, request/response correlation, ACK, and result independent of the original call. ACK-timeout and recipient-disconnect results (#19–#20), sender-disconnect assertions (#21), retry/dedupe (#22–#23), 256 KiB body denial (#24), and 128-send pipelining (#25) remain incomplete.

The local extension accepts safe string or JSON-object bodies under its closed tool schema. Unsafe non-JSON runtime graphs and malformed `re` values fail content-free before IDs or socket work. The sender extension currently accepts exactly the closed correlated result payloads `{message_id,status:"received"}` and `{message_id,status:"timeout",reason:"offline"}`. Every other status/reason pair is an invalid response that terminally closes the retained socket. If #19, #20, or #24 later implements another sender outcome, that work must explicitly expand the result type and allowlist before acceptance; no additional outcome is claimed today. The relay protocol parser remains the strict duplicate/type/depth boundary and forwards an accepted object as the same JSON value. The recipient parser accepts only a string or non-array object and passes sender, message ID, optional valid `re`, and body to the pure renderer. Renderer failure produces neither Pi injection nor ACK and force-settles the retained socket.
</Aside>
...
Two sender outcomes are finalized. An exact recipient ACK returns the correlated `send_result` with `status: "received"`. An exact miss at the current published-session lookup immediately returns the same request and message correlation with `status: "timeout"` and `reason: "offline"`. That miss is a normal machine result, not a local exception or protocol denial, and the sender connection remains open for a later list or send. It reveals only current absence: known-offline and never-published valid opaque destinations have the same result, with no history or existence state.

ACK expiry (#19) and recipient disconnect after a successful lookup (#20) do not yet produce a final machine-readable `send_result`. The relay releases retained work after its exact five-second ACK ceiling and may release it earlier when that found recipient disconnects or the server shuts down; expiry itself returns no sender result. The sender extension then reaches its independent five-second response deadline, rejects the local call with `Relay agent_send response timed out.`, and closes the retained socket. These later failure paths must not be converted into the registry-miss outcome.

The currently enforced shared ceilings are:
...
- exactly five seconds for both the relay's pending ACK wait and the extension's independent response wait;
- 1,024 tracked server connections; and
- one use and exactly ten minutes for a pairing code, with the deadline itself expired.

Sender-disconnect assertions (#21), retry and idempotence (#22), dedupe behavior (#23), the distinct 256 KiB body rule (#24), 128-send pipelining (#25), reconnect (#26), restart authentication (#27), replay behavior (#28), and additional diagnostics (#29) remain deferred. This version publishes no I/O shape or runtime guarantee for those seams.
...
The offline event has no `delivery_id` because generation occurs only after a published recipient is found. Authentication fields including pairing code, nonce, signature, and private key use `<redacted>` markers. Message body is user-supplied content and is fully redacted whether it is a string or object. Public keys, request/message IDs, and opaque route IDs remain visible for support correlation. Structured logs remain sufficient; ticket #18 adds no traces or metrics.
