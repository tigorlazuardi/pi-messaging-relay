...
Operation-level denial is a different state transition: a structurally valid operation reaches the operation dispatcher, a canonical result may be returned, and the parser returns to the next message boundary without closing. Tickets #36 and #13 implement live `list`/`roster` discovery, which remains connected after authentication. Ticket #14 implements the successful idle text path through exact recipient acknowledgment. Ticket #18 still owns offline outcomes, while tickets #19 and #20 own missing-ACK and recipient-disconnect sender results; those failure outcomes remain disconnected.
...
<Aside type="note" title="Implementation boundary">
Ticket #14 implements successful idle string delivery: extension-generated request and message IDs, exact opaque-address routing to a published authenticated session, server-generated delivery ID, `pi.sendUserMessage(body)` attempt without steering options, exact recipient ACK authorization, and correlated `received`. Busy routing (#15), provenance and object rendering (#16), replies (#17), offline/ACK-timeout/recipient-disconnect results (#18–#20), sender-disconnect assertions (#21), retry/dedupe (#22–#23), 256 KiB body denial (#24), and 128-send pipelining (#25) remain incomplete.

The final v1 schema continues to accept string or object bodies. During this staged #14 runtime, the extension rejects a local object-body call before creating IDs or writing a frame. A raw client object body still passes the protocol parser, but the delivery dispatcher creates no offer or pending state and returns no interim `send_result`; the authenticated sender may continue with another operation. A recipient extension that receives an object `message` from a hostile or newer server closes without Pi injection or a `received` ACK. Ticket #16 replaces these guards with the specified provenance header and canonical compact object rendering.
</Aside>
