...
Unknown fields and duplicate JSON keys are rejected. Canonical addresses have the form `<full-cwd>@<hostname>#<uuidv7>`, but clients treat the whole value as opaque and copy it verbatim. Cwd and hostname are display metadata, never authorization inputs. Framing, malformed-envelope, protocol-major, and authentication failures close the socket; operation-level rejection returns a result and preserves the connection.
...
<Tabs>
  ...
  <TabItem label="Hello">
    ```json
    {
      ...
      "payload": {
        "client_public_key": "ed25519:base64-public-key",
...
Fixed v1 ceilings are a 256 KiB body, 512 KiB frame, 5-second ACK deadline, 128 pending sends per sender connection, 1,024 dedupe message IDs per recipient session, and a single-use pairing code that expires after 10 minutes.
