...
Retry idempotence (#22), bounded dedupe retention (#23), the distinct serialized-body limit (#24), and per-sender pipelining capacity (#25) are implemented together. Automatic reconnect (#26), restart authentication (#27), broader restart/reconnect replay assertions (#28), and additional diagnostics (#29) remain deferred. This version adds no reconnect, persistence, offline inbox, replay, or extra model tool.
