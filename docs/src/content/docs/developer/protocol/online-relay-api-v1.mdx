...
<Aside type="note" title="Implementation boundary">
Tickets #14–#16 implement successful string and object delivery: extension-generated request and message IDs, exact opaque-address routing to a published authenticated session, server-generated delivery ID, raw JSON-value object forwarding, deterministic provenance rendering, current recipient-local `ctx.isIdle()` selection, exact one-argument idle injection, exact busy `{ deliverAs: "followUp" }` injection, exact recipient ACK authorization, and correlated `received`. No sender field can select delivery mode or steer current work. Replies (#17), offline/ACK-timeout/recipient-disconnect results (#18–#20), sender-disconnect assertions (#21), retry/dedupe (#22–#23), 256 KiB body denial (#24), and 128-send pipelining (#25) remain incomplete.

The local extension accepts safe string or JSON-object bodies under its closed tool schema. Unsafe non-JSON runtime graphs fail content-free before IDs or socket work. The relay protocol parser remains the strict duplicate/type/depth boundary and forwards an accepted object as the same JSON value. The recipient parser accepts only a string or non-array object and passes sender, message ID, optional valid `re`, and body to the pure renderer. Renderer failure produces neither Pi injection nor ACK and force-settles the retained socket.
</Aside>
