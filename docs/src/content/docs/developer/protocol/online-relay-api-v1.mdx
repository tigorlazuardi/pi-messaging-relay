...
```mermaid
flowchart TD
    O[Operator starts localhost server] --> C[Server prints one-time pairing code]
...
The pairing code and nonce are authentication material and are always redacted from logs. The server persists allowlisted public keys and minimal pairing metadata, but never messages or pending sends.
