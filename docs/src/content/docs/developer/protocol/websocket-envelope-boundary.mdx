...
The production dispatcher intentionally emits no temporary public result. Tickets #13, #14, and #18 connect the accepted roster, delivery, acknowledgment, and offline outcomes. Ticket #24 adds the separate 256 KiB serialized-body rejection. This seam defines and tests dispatch/result behavior with an injected handler without consuming those tickets.
</Aside>
...
|---|---|---|---|
| `list` | none | none | Payload is exactly `{}`. |
