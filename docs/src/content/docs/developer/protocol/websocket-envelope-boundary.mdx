...
The production dispatcher now binds valid `list` operations to live registry reads and bounded `roster` responses. Tickets #14–#18 connect successful delivery, acknowledgment, correlated replies, and immediate offline registry-miss outcomes. Ticket #24 adds the separate 256 KiB serialized-body rejection. This seam continues to define and test generic dispatch/result behavior without consuming those delivery tickets.
</Aside>
